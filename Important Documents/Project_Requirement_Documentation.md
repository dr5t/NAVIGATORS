...
- **AI-Based Speed Estimation:** Estimate vehicle speed/velocity from smartphone IMU data without OBD-II using a robust Temporal Convolutional Network.
- **Vibration & Noise Filtering:** Handle engine vibration, road vibration, potholes, bumps, shocks, and phone movement using non-linear median filtering and Butterworth low-pass filters.
- **Phone Alignment:** Automatically estimate phone pitch, roll, and yaw relative to vehicle motion.
- **Map Matching:** Constrain drifting inertial trajectories to plausible roads using offline maps and geometric road-network information.
- **Vehicle Kinematics:** Use Non-Holonomic Constraints (NHC) and related motion constraints to eliminate lateral slip.
- **GNSS + INS Fusion:** Fuse GNSS and inertial measurements via a 15-state Extended Kalman Filter (EKF) to reduce drift and improve velocity/position estimates.
...
- **100% Offline Edge Execution:** The entire AI model, EKF, and navigation engine must run directly on the mobile device's CPU inside Javascript without requiring any backend servers, internet connection, or cellular network.
...
| **FOG-IMU edge engine capability** | 200 Hz or higher |
